...
// ── Enums ────────────────────────────────────────────────────────────────
export const accountTier = pgEnum("account_tier", ["enterprise", "mid", "smb"]);
...
// Underlying pg enum TYPE is named `outreach_stage` (not `outreach_status`) on
// purpose: the column is named outreach_status, but reusing the same name for
// the type collides with the now-removed pending/approved/sent enum during
// migration. A distinct type name keeps the add-column / drop-old-type ordering
// clean. The DB column and app field stay `outreach_status`.
...
export const telemetryEvents = pgTable(
  ...
  (t) => ({
    ...
    byAccountTime: index("idx_events_account_time").on(
      t.accountId,
      t.occurredAt,
    ),
  }),
...
export const outreach = pgTable("outreach", {
  ...
  incidentStatus: incidentStatus("incident_status").notNull().default("active"),
  ...
  outreachStatus: outreachStage("outreach_status").notNull().default("none"),
