...
0. **Landing + SSO gate** (`/` is public → `/login`) — a one-screen orientation (what it does, who it's for, the one-sentence Aurora pitch) that's **public** so judges read the pitch before signing in, with a themed **"Sign in with SSO"** call to action. The app-level access-code gate guards only the live demo surfaces (`/dashboard`, `/incidents/*`); the landing and login stay open. On-thesis (Sybil's customers *are* SSO vendors) and clearly labelled a demo, not real auth. Signing in resets to calm, warms the cluster, and drops you into the command center — which holds green a beat, then **auto-fires the incident**, so an unattended judge sees the full green→red arc without clicking. Manual Trigger/Reset controls stay for driving it live.
